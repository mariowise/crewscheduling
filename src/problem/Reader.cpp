	...
	while(!f.eof()) {
		...
		if(line.compare("GENERAL") == 0) {
			...
			while(!isPrivate(line)) {
				...
				generalIntervals["maxTimeContinuousDriving"] = (DateTime) dataSep.at(1);	
				generalIntervals["maxTimeDrivingBeforeLunch"] = (DateTime) dataSep.at(2);
				generalIntervals["restLength"] = (DateTime) dataSep.at(3);
				generalIntervals["lunchLength"] = (DateTime) dataSep.at(4);
